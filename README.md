...
| scrollBottomOffset | optional | number | row height | When scrolling down(`deltaY > 0` for the mouse wheel, or when keys `'ArrowDown'` or `'PageDown'` are pressed), if the scroll is >= `container.clientHeight - scrollBottomOffset`, table's scroll will set to the maximum possible | 
