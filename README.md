...
- Crea una variable con la tabla:
    ```javascript
    const testTable = new DataTable(configObjet, data)
    ```

- El parametro `data` es opcional(se puede usar `tableData`).
...
| ------------------------------- | -------------------- | ---------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |---|
| id               | mandatory            | string                 | -               | Selector CSS del elemento del DOM que actua como contenedor de la tabla | - |
| primary | optional | string | "id" | Propiedad que será usada como primary key | - |
| secondary | optional | array ([string]) | - | Propiedades que serán usadas como secondary key. Cualquiera de las propiedades en el array vale para hacer "match" y la comprobación se hará en el orden del array | - |
...
| scrollBottomOffset | optional | number | row height | Cuando ocurre un scroll "hacia abajo"(`deltaY > 0` para la rueda del ratón, o cuando se presionan las teclas `'ArrowDown'` o `'PageDown'`), si el scroll es >= que `container.clientHeight - scrollBottomOffset`, el scroll de la tabla se setea directamente al máximo posible | - 
...
| first_column_count                  | optional             | number                | 0           | Primer valor que se muestra en la header de las filas si showRowHeaders está activado |-                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
